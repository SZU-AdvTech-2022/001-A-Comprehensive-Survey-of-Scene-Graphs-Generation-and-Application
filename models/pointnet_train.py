...
def load_pretained_cls_model(model, local_rank):
    # load pretrained pointnet_cls model [ relPointNet ver. ]
    pretrained_dict = torch.load(os.path.join(CONF.PATH.BASE, './pointnet_cls_best_model.pth'), map_location=lambda storage, loc: storage.cuda(local_rank))["model_state_dict"]
...
class SGPN(nn.Module):
    def __init__(self, use_pretrained_cls, gconv_dim=128, gconv_hidden_dim=512,
        ...
        if use_pretrained_cls:
            load_pretained_cls_model(self.objPointNet.cuda(local_rank), local_rank)
        ...
        if use_pretrained_cls:
            load_pretained_cls_model(self.relPointNet.cuda(local_rank), local_rank)
    ...
    def forward(self, data_dict):
        ...
        obj_vecs, _, tf1 = self.objPointNet(objects_pc)  # obj_vecs[num_object, H] tf1[object_num, 64, 64]表示转换矩阵（STNkd）
