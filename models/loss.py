...
class CrossEntropyFocalLoss(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        target = target.view(-1, 1)  # [9，1]
        logits = torch.sigmoid(logits)  # [9, 160] 缩小特征向量的取值范围 

        pt = F.softmax(logits, 1)  # [9, 160]
        pt = pt.gather(1, target).view(-1)  # [9] 取出target对应标签位置的logit值
        log_gt = torch.log(pt)
...
class PerClassBCEFocalLosswithLogits(nn.Module):    # namely multi-label classification
    ...
    def forward(self, logits, target):
        ...
        if not self.training:
            logits = torch.clamp(logits, -5, 5)  # avoid overflow after sigmoid computation in the val phase, remains for observation
        logits = torch.sigmoid(logits)  # [72,27]
        alpha = self.alpha
        gamma = self.gamma
