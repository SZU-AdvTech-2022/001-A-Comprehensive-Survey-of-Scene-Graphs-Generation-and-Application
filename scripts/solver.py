...
class Solver():
    def __init__(self, args, model, dataloader, datasampler, optimizer, stamp, val_step=10,
    ...
    def __call__(self, epoch, verbose):
        ...
        for epoch_id in range(epoch):
            try:
                ...
                # feed
                self.datasampler["train"].set_epoch(epoch)

                self._feed(self.dataloader["train"], "train", epoch_id, local_rank)

                # save model
                if local_rank==0  and epoch_id % 10 == 0:
    ...
    def _feed(self, dataloader, phase, epoch_id, local_rank):
        # switch mode
        self._set_phase(phase)  # 选择是train还是val
        # re-init log
        self._reset_log(phase)  # 初始化log
        # change dataloader
        ...
        for data_dict in dataloader:
            ...
            for key in data_dict:
                if key != "scan_id":
                    data_dict[key] = data_dict[key].cuda(local_rank)
            ...
            # with torch.autograd.set_detect_anomaly(True):  # 正向传播时
                # forward
            ...
            self.log[phase]["loss"].append(self._running_log["loss"])
            ...
            if phase == "train" and local_rank == 0:
                ...
                if self._global_iter_id % self.val_step == 0:
                    print("evaluating...")
                    # val
                    self._feed(self.dataloader["val"], "val", epoch_id, local_rank=local_rank)
                    self._dump_log("val")
    ...
    def _compute_loss(self, data_dict, lambda_obj=0.1, mat_diff_loss_scale=0.001):
        ...
        # total loss
        # L_obj_sum = np.array([i.cpu().detach().numpy() for i in Focal_Lobj]).sum()
        # L_pred_sum = np.array([i.cpu().detach().numpy() for i in Focal_Lpred]).sum()
        # Loss = lambda_obj * L_obj_sum + L_pred_sum

        for i in range(len(Focal_Lobj)):
            Loss.append(lambda_obj * Focal_Lobj[i] + Focal_Lpred[i])
        
        L_obj_mean = np.array([i.cpu().detach().numpy() for i in Focal_Lobj]).mean()
        L_pred_mean = np.array([i.cpu().detach().numpy() for i in Focal_Lpred]).mean()
        Loss_mean = np.array([i.cpu().detach().numpy() for i in Loss]).mean()

        # dump
        self._running_log["loss"] = Loss_mean
        self._running_log["L_obj"] = L_obj_mean    # average object classification loss in one scene
        self._running_log["L_pred"] = L_pred_mean
        data_dict["loss"] = Loss
        data_dict["L_obj"] = L_obj_mean
        data_dict["L_pred"] =  L_pred_mean
