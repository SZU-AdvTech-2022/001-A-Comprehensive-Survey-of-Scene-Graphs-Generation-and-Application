...
def get_eval(data_dict):
    ...
    for i in range(batch_size):
        ...
        pairs = data_dict["pairs"][i][:predicate_count].cpu().numpy()  # 物体标签对  [72, 2]
        edges = data_dict["edges"][i][:predicate_count].cpu().numpy()  # 物体索引对
        triples = data_dict["triples"][i].cpu().numpy()  # 三元组[sub, obj, relation]
        ...
        obj_scores_per_rel = object_logits[edges].prod(1)  # 取一对的索引然后计算score值的乘积
        overall_scores = obj_scores_per_rel[:, None] * predicate_pred  # 再和relaition的score值相乘
        score_inds = argsort_desc(overall_scores)[:100]  # 取前一百个最可能的三元组（注意这里两个物体之间可能被预测多种关系）
